...
def build(system: str, user: str, max_tokens: int,
    ...
    if name == "ollama":
        ...
        payload = {
            ...
            "options": {
                # По умолчанию Ollama даёт 2048 токенов контекста. Методика
                # канала весит тысячи токенов и просто не влезет — обрежется
                # молча, без ошибки, и модель ответит мимо задания.
                "num_ctx": env_int("OLLAMA_CTX", 16384),
