...
# Документ с методикой человек чаще всего хранит целиком: сверху пояснения
# «как пользоваться», сам промпт выделен маркерами. В модель должен уходить
# только промпт, иначе она получает инструкцию для человека.
МАРКЕРЫ = (
...
def prompt_body(text: str) -> str:
    """Тело промпта: блок между маркерами, если он есть, иначе весь документ.

    Маркеры почти всегда упоминаются и в инструкции «как пользоваться» в
    начале документа. Первая же найденная пара тогда даёт пустышку в пару
    символов, поэтому берём самый длинный блок из всех пар.
    """
    лучший = ""
    for начало, конец in МАРКЕРЫ:
        i = text.find(начало)
        while i >= 0:
            j = text.find(конец, i + len(начало))
            кусок = text[i + len(начало):j if j > 0 else None]
            кусок = кусок.strip(chr(96) + " " + chr(10) + chr(13))
            if len(кусок) > len(лучший):
                лучший = кусок
            i = text.find(начало, i + len(начало))
    return лучший or text.strip()

def read_prompt(project, name: str, fallback: str | None = None) -> tuple[str, str]:
    """Методика шага: (текст промпта, путь-источник для печати).

    fallback — имя запасного файла: у шага может не быть своей методики, тогда
    берётся общая. Если нет и её, вернётся пустая строка, и шаг решит сам.
    """
    for имя in (name, fallback):
        if not имя:
            continue
        try:
            путь, уровень = project.resolved(имя)
        except SystemExit:
            continue
        return prompt_body(read_text(путь)), f"{путь} ({уровень})"
    return "", ""
