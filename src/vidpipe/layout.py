...
def разобрать(папка: Path) -> dict:
    ...
    видеоф = sorted((x for x in файлы if x.suffix.lower() in ВИДЕО),
                    key=lambda x: x.name)
    карт_a, карт_b, прочие, вторые = {}, {}, [], []

    def лучше(новый: Path, прежний: Path) -> bool:
        """Кого оставить, если на один номер претендуют двое.

        Так бывает, когда в папке лежат две партии генерации: вторая
        закачка добавляет к имени « (1)», а браузер кладёт её в подпапку.
        Раньше побеждал тот, кто позже попался при обходе, — то есть выбор
        между партиями был случайным. Теперь он явный: файл прямо в clips
        старше вложенного, имя без « (N)» старше имени с ним.
        """
        глубже = len(новый.parts) > len(прежний.parts)
        мельче = len(прежний.parts) > len(новый.parts)
        if мельче:
            return True
        if глубже:
            return False
        коп_н = " (" in новый.stem
        коп_п = " (" in прежний.stem
        return коп_п and not коп_н
    ...
    for x in файлы:
        ...
        if not m:
            прочие.append(x)
            continue
        куда = карт_a if m.group(2) == "a" else карт_b
        н = int(m.group(1))
        прежний = куда.get(н)
        if прежний is None:
            куда[н] = x
        elif лучше(x, прежний):
            вторые.append(прежний)
            куда[н] = x
        else:
            вторые.append(x)
    if not карт_a and прочие:                       # без вариантов _a/_b
        # Так выглядит уже разложенная папка: имена вида `023-Title.jpeg`,
        # без `_a`/`_b`. Взятое отсюда обязательно вычесть из `прочие`:
        # иначе те же файлы попадут и в план, и в «варианты», а `разложить`
        # уносит варианты первым делом — и переименовывать становится нечего.
        # Живой случай: 35 картинок уехали в `_варианты`, сборка упала на
        # первом же файле, в clips остались одни видео.
        взяты = []
        for x in прочие:
            ...
            if m:
                ...
                взяты.append(x)
        прочие = [x for x in прочие if x not in взяты]
    ...
    return {
        ...
        "план": план,
        "слабо": слабо,
        "варианты": list(карт_b.values()) + вторые + прочие,
...
def разложить(папка: Path, разбор: dict | None = None) -> dict:
    ...
    if not р.get("годится"):
        return р
    clips = папка / "clips"
    # Ни одного движения, пока не убедились, что план исполним целиком.
    # Раскладка, оборвавшаяся на середине, оставляет папку в состоянии,
    # из которого её уже не соберёт ни одна команда.
    пропали = [п["файл"] for п in р["план"] if not п["файл"].exists()]
    if пропали:
        return dict(р, годится=False,
                    почему="в плане %d файлов, которых нет на диске: %s"
                           % (len(пропали), пропали[0].name))
    врем = {}
    ...
    for t, ц in врем.items():
        t.rename(ц)
    # В сторону — только после того, как переименование прошло целиком.
    if р["варианты"]:
        вар = папка / "_варианты"
