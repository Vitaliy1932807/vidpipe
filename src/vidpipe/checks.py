...
def check_bible(project) -> list[Issue]:
    ...
    # Модель охотно копирует идентификаторы из примеров задания вместо того,
    # чтобы взять их из сценария. Живой случай: в библии для ролика про
    # Тенерифе оказались PETR_01 и MOTOVOZ_01 из примера в промпте шага.
    from .steps import bible as шаг
    из_примера = set(re.findall("[A-ZА-Я][A-ZА-Я0-9_]{2,}", шаг.SYSTEM))
    списанные = [и for и in list(герои) + list(предметы) if и in из_примера]
    if списанные:
        out.append(Issue("stop",
                         f"идентификаторы списаны из примера: {перечислить(списанные)}",
                         "перепиши библию руками или vidpipe -s bible -f: "
                         "имена должны браться из сценария"))
...
def check_flow(project) -> list[Issue]:
    ...
    # Только люди. Предмет в сцене без людей — норма и есть смысл библии:
    # журнал и молоток должны выглядеть одинаково во всех кадрах, в том числе
    # там, где в кадре никого нет.
    люди_книги = set(глоб.get("characters") or {})
    призраки = [с.get("scene") for с in сцены
                if set(с.get("characters") or []) & люди_книги
