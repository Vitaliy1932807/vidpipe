...
Два слоя, и это разделение принципиальное.

Первый слой, код: считает, сверяет, сравнивает с соседними файлами. Он либо
прав, либо нет, и ему можно доверить остановку конвейера.

Второй слой, субагент-критик: читает результат и методику канала и говорит,
что не так по смыслу. Он полезен, но ошибается, поэтому его находки всегда
warn и никогда не останавливают работу. Локальная модель на 14B, заблокировав
верный сценарий, обошлась бы дороже, чем пропущенная шероховатость.
"""
...
def postflight(project, step: str, strict: bool = True) -> None:
    ...
    for fn in POSTFLIGHT.get(step, []):
        issues += fn(project)
    # останавливать вправе только код: находки критика всегда совещательные
    из_кода = [i for i in issues if i.level == "stop"]

    from .critic import review_output
    issues += review_output(project, step)
    for i in issues:
        print(i.show())

    if из_кода and strict:
        raise SystemExit(
